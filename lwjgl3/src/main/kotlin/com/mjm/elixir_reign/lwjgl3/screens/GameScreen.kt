...
class GameScreen(private val game: Main) : ScreenAdapter() {
    ...
    private val placementTouchStartTimes = mutableMapOf<Int, Long>()
    private val placementTouchStartPositions = mutableMapOf<Int, Vector2>()
    private val placementTouchDraggedPointers = mutableSetOf<Int>()
    ...
    private val input = object : InputAdapter() {
        ...
        override fun touchDown(screenX: Int, screenY: Int, pointer: Int, button: Int): Boolean {
            ...
            if (buildPlacementHandler.isPlacementModeActive()) {
                placementTouchStartTimes[pointer] = System.currentTimeMillis()
                placementTouchStartPositions[pointer] = Vector2(screenX.toFloat(), screenY.toFloat())
                placementTouchDraggedPointers.remove(pointer)
        ...
        override fun touchDragged(screenX: Int, screenY: Int, pointer: Int): Boolean {
            ...
            if (buildPlacementHandler.isPlacementModeActive()) {
                val placementStart = placementTouchStartPositions[pointer]
                if (placementStart != null) {
                    val deltaX = screenX.toFloat() - placementStart.x
                    val deltaY = screenY.toFloat() - placementStart.y
                    if (deltaX * deltaX + deltaY * deltaY >= PLACEMENT_DRAG_THRESHOLD_PX * PLACEMENT_DRAG_THRESHOLD_PX) {
                        placementTouchDraggedPointers += pointer
                    }
                }
            }
        ...
        override fun touchUp(screenX: Int, screenY: Int, pointer: Int, button: Int): Boolean {
            ...
            if (buildPlacementHandler.isPlacementModeActive()) {
                val placementStartTime = placementTouchStartTimes[pointer]
                val isShortClick = placementStartTime != null &&
                    !placementTouchDraggedPointers.contains(pointer) &&
                    System.currentTimeMillis() - placementStartTime <= PLACEMENT_TAP_MAX_DURATION_MS

                if (button == Input.Buttons.RIGHT && isShortClick) {
                    Shop.hide()
                    buildPlacementHandler.cancelPlacement()
                } else if (button == Input.Buttons.LEFT && isShortClick) {
                ...
                placementTouchStartTimes.remove(pointer)
                placementTouchStartPositions.remove(pointer)
                placementTouchDraggedPointers.remove(pointer)
                if (activeTouches.size >= 2) {
                    beginPinch()
                } else {
    ...
    override fun show() {
        ...
        debugRenderer = GameScreenDebugRenderer(gameWorld.coreEngine, worldRenderer.geometry())
    ...
    private fun requestBuildingPlacement(
        row: Int,
        col: Int,
    ...
    private fun positionBuildingPanelAbove(entity: Entity) {
        ...
        val screen = camera.project(com.badlogic.gdx.math.Vector3(anchorX, anchorY, 0f))
        val padding = 10f
    ...
    companion object {
        ...
        private const val PLACEMENT_TAP_MAX_DURATION_MS = 220L
        private const val PLACEMENT_DRAG_THRESHOLD_PX = 12f
    }
